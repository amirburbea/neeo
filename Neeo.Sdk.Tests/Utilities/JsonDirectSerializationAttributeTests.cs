...
public sealed class JsonDirectSerializationAttributeTests
{
    ...
    {
        string text = JsonSerializer.Serialize((IWithAttribute)Foo.Instance, JsonSerialization.WebOptions);
    ...
    {
        string text = JsonSerializer.Serialize((IWithoutAttribute)Foo.Instance, JsonSerialization.WebOptions);
        Assert.Equal("{}", text);
    }
