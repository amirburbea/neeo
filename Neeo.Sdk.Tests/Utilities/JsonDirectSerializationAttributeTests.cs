...
public sealed class JsonDirectSerializationAttributeTests
{
    ...
    {
        string text = JsonSerializer.Serialize((IWithAttribute)Foo.Instance, JsonSerializerOptions.Web);
    ...
    {
        string text = JsonSerializer.Serialize((IWithoutAttribute)Foo.Instance, JsonSerializerOptions.Web);
        Assert.Equal("{}", text);
    }
