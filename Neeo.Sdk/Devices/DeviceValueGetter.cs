...
public static class DeviceValueGetter
{
    ...
    public static DeviceValueGetter<TValue> FromValue<TValue>(TValue value)
        where TValue : notnull => (_, _) => Task.FromResult(value);
}
...
public delegate Task<TValue> DeviceValueGetter<TValue>(string deviceId, CancellationToken cancellationToken = default)
